    ...
    .then(deletedSprint => {
        if (deletedSprint && deletedSprint.issues !== []) {
            deletedSprint.issues.map(issueId => {
                Issue.findOneAndUpdate({ _id: issueId }, { isPartOfSprint: false }, {new: true})
                    .catch(err1 => {
                        res.send({
                            success: false,
                            isError: true,
...
export const addIssueToSprint = (req, res) => {
    Issue.findOne({ _id: req.body.issueId })
    .then(issue => {
        if (issue) {
            Sprint.findOne({ _id: req.body.sprintId })
            .then(sprintFound => {
                if (sprintFound) {
...
export const removeIssueFromSprint = (req, res) => {
    let sprint = {}
